...
def get_player_filters_keyboard():
    ...
    for i, team in enumerate(teams[:5]):
        team_row1.append(InlineKeyboardButton(team, callback_data=f"player_filter_team_{team}"))
    
    for i, team in enumerate(teams[5:]):
        team_row2.append(InlineKeyboardButton(team, callback_data=f"player_filter_team_{team}"))
    ...
    for player_type in player_types:
        type_row.append(InlineKeyboardButton(player_type, callback_data=f"player_filter_type_{player_type}"))
...
def get_player_details_keyboard(player: Dict[str, Any]):
    ...
    if not player.get("in_user_team", False):
        keyboard.append([
            InlineKeyboardButton("Add to Team", callback_data=f"player_add_{player['id']}")
        ])
    else:
        keyboard.append([
            InlineKeyboardButton("Remove from Team", callback_data=f"player_remove_{player['id']}")
        ])
    
    # Bid button (if auction is active)
    if player.get("auction_status") == "active":
        keyboard.append([
            InlineKeyboardButton("Place Bid", callback_data=f"auction_bid_{player['id']}")
        ])
    
    # Set as captain button (if in team)
    if player.get("in_user_team", False):
        keyboard.append([
            InlineKeyboardButton("Set as Captain", callback_data=f"player_captain_{player['id']}")
        ])
