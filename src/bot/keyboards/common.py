...
def get_pagination_keyboard(current_page, total_pages, base_callback="nav_page"):
    ...
    navigation = []
    if current_page > 0:
        navigation.append(InlineKeyboardButton("« Prev", callback_data=f"{base_callback}_{current_page-1}"))
    
    navigation.append(InlineKeyboardButton(f"{current_page+1}/{total_pages}", callback_data="nav_noop"))
    
    if current_page < total_pages - 1:
        navigation.append(InlineKeyboardButton("Next »", callback_data=f"{base_callback}_{current_page+1}"))
