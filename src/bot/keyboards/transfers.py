...
def get_transfer_menu_keyboard(window_status: str) -> InlineKeyboardMarkup:
    ...
    if window_status == "active":
        keyboard = [
            [
                InlineKeyboardButton(
                    "View Available Players", callback_data="view_available_players"
                )
            ],
            [
                InlineKeyboardButton(
                    "List Player for Sale", callback_data="list_player_for_sale"
                )
            ],
        ...
        keyboard = [
            [
                InlineKeyboardButton(
                    "View Last Window Results", callback_data="view_transfer_results"
                )
            ],
            [InlineKeyboardButton("« Back to Menu", callback_data="nav_back_main")],
        ]
...
def get_player_listing_keyboard(
    listing: Dict[str, Any], user: Dict[str, Any]
    ...
    if listing.get("seller_id") == user.get("id"):
        ...
        if user_has_bid:
            ...
            keyboard = [
                [
                    InlineKeyboardButton(
                        "Place Bid", callback_data=f"bid_on_listing_{listing.get('id')}"
                    )
                ]
            ]
    ...
    keyboard.append(
        [
            InlineKeyboardButton(
                "« Back to Available Players", callback_data="view_available_players"
            )
        ]
    )
...
def get_bid_confirmation_keyboard(
    listing_id: int, bid_amount: Decimal, is_free_bid: bool
) -> InlineKeyboardMarkup:
