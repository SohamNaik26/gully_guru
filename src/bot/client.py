...
class APIClient:
    ...
    def __init__(self, base_url: str = None):
        ...
        self.base_url = base_url or f"http://{settings.API_HOST}:{settings.API_PORT}/api"
    ...
    async def get_user(self, telegram_id: int) -> Optional[Dict[str, Any]]:
        ...
        try:
            response = await self.client.get(f"{self.base_url}/users/telegram/{telegram_id}")
    ...
    async def get_player_stats(self, player_id: int) -> Optional[Dict[str, Any]]:
        ...
        try:
            response = await self.client.get(f"{self.base_url}/players/{player_id}/stats")
            if response.status_code == 200:
                return response.json()
            return None
        except Exception as e:
    ...
    async def get_user_team(self, user_id: int, game_id: Optional[int] = None) -> Dict[str, Any]:
        """Get a user's team."""
        try:
            params = {}
            if game_id:
            ...
            response = await self.client.get(f"{self.base_url}/users/{user_id}/team", params=params)
            return response
        except Exception as e:
            logger.error(f"Error getting user team: {e}")
            return {"success": False, "error": str(e)}
    
    async def buy_player(self, user_id: int, player_id: int, price: Decimal) -> Optional[Dict[str, Any]]:
    ...
    async def get_auction_status(self, game_id: Optional[int] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            response = await self.client.get(f"{self.base_url}/fantasy/auction/current", params=params)
            return response
        except Exception as e:
            logger.error(f"Error getting auction status: {e}")
            return None
    
    async def place_bid(self, user_id: int, player_id: int, amount: Decimal) -> Optional[Dict[str, Any]]:
    ...
    # Match endpoints
    async def get_matches(self, upcoming: bool = True, limit: int = 5) -> List[Dict[str, Any]]:
    ...
    async def submit_prediction(self, user_id: int, match_id: int, prediction: str) -> bool:
    ...
    async def get_leaderboard(self, limit: int = 10) -> List[Dict[str, Any]]:
        ...
        try:
            response = await self.client.get(f"{self.base_url}/stats/leaderboard", params={"limit": limit})
            if response.status_code == 200:
                return response.json()
    ...
    async def assign_player_to_user(self, user_id: int, player_id: int, price: float) -> Dict[str, Any]:
    ...
    # Transfer endpoints
    async def get_transfer_listings(self, status: str = "available", window_id: int = None) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            response = await self.client.get(
                f"{self.base_url}/transfers/listings",
                params=params
            )
    ...
    async def get_user_listings(self, user_id: int, status: str = "available") -> List[Dict[str, Any]]:
    ...
    async def create_transfer_listing(self, player_id: int, min_price: Decimal, transfer_window_id: int) -> Optional[Dict[str, Any]]:
    ...
    async def place_transfer_bid(self, listing_id: int, bid_amount: Decimal) -> Dict[str, Any]:
    ...
    async def get_current_transfer_window(self) -> Optional[Dict[str, Any]]:
        ...
        try:
            response = await self.client.get(
                f"{self.base_url}/transfers/current"
            )
    ...
    async def validate_user_team(self, user_id: int) -> Dict[str, Any]:
        ...
        try:
            response = await self.client.post(
                f"{self.base_url}/fantasy/validate-team",
                params={"user_id": user_id}
            )
            
            if response.status_code == 200:
                return response.json()
            return {"valid": False, "message": response.json().get("detail", "Unknown error")}
        except Exception as e:
            logger.error(f"Error validating user team: {e}")
            return {"valid": False, "message": str(e)}

    # Game management endpoints
    async def create_gully(self, name: str, telegram_group_id: int, start_date: datetime, end_date: datetime) -> Dict[str, Any]:
    ...
    async def get_gully_by_group(self, group_id: int) -> Dict[str, Any]:
        ...
        try:
            response = await self.client.get(f"{self.base_url}/gullies/group/{group_id}")
            return response.json()
        except Exception as e:
            logger.error(f"Error getting gully by group: {e}")
            return None

    async def join_game(self, game_id: int, user_id: int, team_name: str) -> Dict[str, Any]:
    ...
    async def get_game_participants(self, game_id: int) -> List[Dict[str, Any]]:
        ...
        try:
            response = await self.client.get(f"{self.base_url}/games/{game_id}/participants")
            return response.json()
        except Exception as e:
