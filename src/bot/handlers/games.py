...
from src.bot.keyboards.gullies import get_gully_management_keyboard, get_gully_list_keyboard
...
async def create_gully_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
...
async def join_gully_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if update.effective_chat.type in ["group", "supergroup"]:
        ...
        await update.message.reply_text(
            "You are participating in the following gullies:",
...
async def my_gullies_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
...
async def switch_gully_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    for gully in user_gullies:
        keyboard.append([
            InlineKeyboardButton(
                gully.get("name"),
                callback_data=f"switch_gully_{gully.get('id')}"
            )
        ])
    
    await update.message.reply_text(
        "Select a gully to switch to:",
        reply_markup=InlineKeyboardMarkup(keyboard)
    )
    
    return GULLY_SELECTION

async def gully_selection_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if not selected_gully:
        await query.edit_message_text(
            "Gully not found. Please try again."
        )
        return ConversationHandler.END
...
async def gully_info_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
...
def register_handlers(application):
    ...
    create_gully_conv = ConversationHandler(
        ...
        states={
            GULLY_NAME: [MessageHandler(filters.TEXT & ~filters.COMMAND, gully_name_handler)]
        },
    ...
    switch_gully_conv = ConversationHandler(
        ...
        states={
            GULLY_SELECTION: [CallbackQueryHandler(gully_selection_handler, pattern=r"^switch_gully_\d+$")]
        },
        fallbacks=[CommandHandler("cancel", lambda u, c: ConversationHandler.END)]
    )
