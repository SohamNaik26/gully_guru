...
async def transfers_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if transfer_window.get("status") == "active":
        ...
        end_time = datetime.fromisoformat(
            transfer_window.get("end_time").replace("Z", "+00:00")
        )
        ...
        start_time = datetime.fromisoformat(
            transfer_window.get("start_time").replace("Z", "+00:00")
        )
        now = datetime.now(pytz.UTC)
    ...
    if data == "view_available_players":
        ...
        keyboard = InlineKeyboardMarkup(
            [
                [
                    InlineKeyboardButton(
                        f"{i}. {listing.get('player', {}).get('name')}",
            ...
            + [
                [
                    InlineKeyboardButton(
                        "« Back to Transfers", callback_data="nav_transfers"
                    )
                ]
            ]
        )
        ...
        keyboard = InlineKeyboardMarkup(
            [
                [
                    InlineKeyboardButton(
                        f"₹{base_price} cr", callback_data=f"set_price_{base_price}"
                    ),
                    InlineKeyboardButton(
                        f"₹{base_price + Decimal('0.5')} cr",
                        callback_data=f"set_price_{base_price + Decimal('0.5')}",
                    ),
                ],
                [
                    InlineKeyboardButton(
                        f"₹{base_price + Decimal('1.0')} cr",
                        callback_data=f"set_price_{base_price + Decimal('1.0')}",
                    ),
                    InlineKeyboardButton(
                        f"₹{base_price + Decimal('2.0')} cr",
                        callback_data=f"set_price_{base_price + Decimal('2.0')}",
                    ),
                ],
        ...
        keyboard = InlineKeyboardMarkup(
            [
                [
                    InlineKeyboardButton(
                        f"₹{min_price} cr", callback_data=f"set_bid_{min_price}"
                    ),
                    InlineKeyboardButton(
                        f"₹{min_price + Decimal('0.5')} cr",
                        callback_data=f"set_bid_{min_price + Decimal('0.5')}",
                    ),
                ],
                [
                    InlineKeyboardButton(
                        f"₹{min_price + Decimal('1.0')} cr",
                        callback_data=f"set_bid_{min_price + Decimal('1.0')}",
                    ),
                    InlineKeyboardButton(
                        f"₹{min_price + Decimal('2.0')} cr",
                        callback_data=f"set_bid_{min_price + Decimal('2.0')}",
                    ),
                ],
                [
                    InlineKeyboardButton(
                        "Cancel", callback_data=f"view_player_listing_{listing_id}"
                    )
                ],
            ]
        )
        ...
        bid = await api_client.place_transfer_bid(
            listing_id=listing_id, bid_amount=bid_amount, is_free_bid=is_free_bid
        )
        ...
        if bids:
            ...
            for i, bid in enumerate(
                sorted(
                    bids,
                    key=lambda x: Decimal(str(x.get("bid_amount", 0))),
                    reverse=True,
                ),
                1,
            ):
            ...
            highest_bid = sorted(
                bids, key=lambda x: Decimal(str(x.get("bid_amount", 0))), reverse=True
            )[0]
            ...
            keyboard = InlineKeyboardMarkup(
                [
                    ...
                    [
                        InlineKeyboardButton(
                            "« Back to My Listings", callback_data="view_my_listings"
                        )
                    ],
                ]
            )
        ...
        keyboard = InlineKeyboardMarkup(
            [
                [
                    InlineKeyboardButton(
                        f"{i}. {bid.get('listing', {}).get('player', {}).get('name')}",
...
transfer_listing_handler = ConversationHandler(
    ...
    states={
        LISTING_PRICE: [
            CallbackQueryHandler(
                handle_transfer_callback, pattern=r"^set_price_\d+(\.\d+)?$"
            ),
...
transfer_bid_handler = ConversationHandler(
    ...
    states={
        BID_AMOUNT: [
            CallbackQueryHandler(
                handle_transfer_callback, pattern=r"^set_bid_\d+(\.\d+)?$"
            ),
            CallbackQueryHandler(
                handle_transfer_callback, pattern=r"^view_player_listing_\d+$"
            ),
        ]
    },
    fallbacks=[CommandHandler("cancel", lambda u, c: ConversationHandler.END)],
...
async def schedule_transfer_deadline_job(context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if end_time <= now:
        ...
        logger.info(
            f"Scheduled transfer deadline job for {end_time} ({seconds_until_end} seconds from now)"
        )
...
async def process_transfer_deadline(context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not result or not result.get("success"):
        logger.error(
            f"Failed to process transfer deadline: {result.get('error', 'Unknown error')}"
        )
        return
        ...
        try:
            ...
            if sold:
                ...
                for transfer in sold:
                    ...
                    message += f"• {player.get('name')} to {buyer.get('username')} for ₹{price} cr\n"
                message += "\n"
            ...
            if bought:
                ...
                for transfer in bought:
                    ...
                    message += f"• {player.get('name')} from {seller.get('username')} for ₹{price} cr\n"
                message += "\n"
            ...
            # Send notification
            await context.bot.send_message(
                chat_id=user.get("telegram_id"), text=message, parse_mode="Markdown"
            )
        except Exception as e:
            logger.error(
                f"Error notifying user {user.get('id')} about transfer results: {e}"
            )


async def view_transfer_results_command(
    update: Update, context: ContextTypes.DEFAULT_TYPE
) -> None:
    ...
    # Format message with transfer results
    message = "📊 *Your Last Transfer Window Results* 📊\n\n"
