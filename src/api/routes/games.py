...
from src.api.schemas.gullies import GullyCreate, GullyResponse, GullyParticipantCreate, GullyParticipantResponse
...
async def get_gully_by_group(
    ...
    gully = session.exec(select(Gully).where(Gully.telegram_group_id == group_id)).first()
...
@router.get("/gullies/{gully_id}/participants", response_model=List[GullyParticipantResponse])
